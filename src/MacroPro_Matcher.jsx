// MacroPro v2.4.2 — Búsqueda Rápida: precio/m² directo sin multiplicar — Build 2026-03-23
        ...
        const mapped = dataRows
          ...
          .map((row, i) => {
            ...
            return {
              ...
              desarrollo: String(getVal(row, "Desarrollo / Proyecto") || "").trim(),
              tipo: String(getVal(row, "Tipo de Inventario") || "").trim(),
              ciudad: String(getVal(row, "Ciudad / Municipio") || "").trim(),
              estado: String(getVal(row, "Estado") || "").trim(),
              colonia: String(getVal(row, "Colonia / Corredor") || "").trim(),
              uso: String(getVal(row, "Uso de Suelo") || "").trim(),
              sup_m2: sup,
              precio_m2: pm2,
              precio_total: total,
...
Rankea los ${top.length} clientes mayor a menor score.`;
  ...
  const ViewSearchLots = () => {
    ...
    const searchResults = source.filter(l => {
      ...
      if (searchPrecioMin && l.precio_m2 < parseFloat(searchPrecioMin)) return false;
      if (searchPrecioMax && l.precio_m2 > parseFloat(searchPrecioMax)) return false;
      return true;
    });
      ...
      <div style={s.page}>
        ...
        <div style={{ background:`linear-gradient(135deg,${B.navy}15,${B.navy}08)`, borderRadius:16, padding:"24px 28px", marginBottom:24, border:`2px solid ${B.gold}44` }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(140px, 1fr))", gap:12 }}>
            ...
            <div>
              <label style={{ fontSize:11, fontWeight:700, color:B.grey4, display:"block", marginBottom:6, textTransform:"uppercase", letterSpacing:0.8 }}>Precio/m² mín ($)</label>
              <input style={{ ...s.input, width:"100%" }} type="number" placeholder="Ej: 5000" value={searchPrecioMin} onChange={e => setSearchPrecioMin(e.target.value)} />
            </div>
            <div>
              <label style={{ fontSize:11, fontWeight:700, color:B.grey4, display:"block", marginBottom:6, textTransform:"uppercase", letterSpacing:0.8 }}>Precio/m² máx ($)</label>
              <input style={{ ...s.input, width:"100%" }} type="number" placeholder="Ej: 12000" value={searchPrecioMax} onChange={e => setSearchPrecioMax(e.target.value)} />
            </div>
          </div>
        </div>
